...
export CODEENGINE_APP_CPU_CONFIG=0.25
export CODEENGINE_APP_MEMORY_CONFIG=1G
export CODEENGINE_APP_MAX_SCALE=1
export CODEENGINE_APP_MIN_SCALE=0
export CODEENGINE_APP_PORT=3000
export CODEENGINE_PROJECT_NAMESPACE=""

export COMMIT_ID=""
export DOCKERFILE_NAME="Dockerfile_custom_grafana"
export FOLDERNAME=""
...
function build_and_push_container () {
    ...
    echo "****** BUILD *********"
    cd "$HOME_PATH"/../../code
    ${CONTAINER_RUNTIME} build -f "$HOME_PATH"/../../code/docker/"$DOCKERFILE_NAME" -t "$CODEENGINE_APP_IMAGE_URL" .
    cd "$HOME_PATH"
...
function log_deployment_configuration_last(){
    ...
    # remove all comments of the envirement configuration and save in all
    sed 's/\APP_APIKEY=.*/APP_APIKEY=/' $HOME_PATH/../../code/.env  > $HOME_PATH/../../deployment-log/last/.env
...
function build_and_push_container_reuse () {
    ...
    DESTINATION_PATH=$TEMP_REUSE_FOLDER/shelly-cloudant-grafana/code
    cp $SOURCE_PATH/$REUSE_REPO_ENV_NAME $DESTINATION_PATH/.env

    # Build from restore code
    cd "$TEMP_REUSE_FOLDER"/shelly-cloudant-grafana
    ${CONTAINER_RUNTIME} build -f "$TEMP_REUSE_FOLDER"/shelly-cloudant-grafana/code/docker/"$DOCKERFILE_NAME" -t "$CODEENGINE_APP_IMAGE_URL" .
    ...
    CURR_CONTAINER_IMAGE=$(ibmcloud cr image-list | grep $CI_TAG | awk '{print $2;}')
    if [ "$CI_TAG" != "$CURR_CONTAINER_IMAGE" ]; then
        ${CONTAINER_RUNTIME} login -u iamapikey -p $IBM_CLOUD_API_KEY $CR_REGION 
        ${CONTAINER_RUNTIME} push "$CODEENGINE_APP_IMAGE_URL"
    else
        echo "Container exists: ($CODEENGINE_APP_IMAGE_URL)"
    fi
